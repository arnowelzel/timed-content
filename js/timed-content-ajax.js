jQuery( document ).ready(
	function() {
		...
		function getArgs() {
			...
			args.im = 1;
			if ( 0 === args.fsel ) {
				args.im = jQuery( 'input#timed_content_rule_hourly_num_of_hours' ).val(); }
			if ( 1 == args.fsel ) {
				args.im = jQuery( 'input#timed_content_rule_daily_num_of_days' ).val(); }
			if ( 2 == args.fsel ) {
				args.im = jQuery( 'input#timed_content_rule_weekly_num_of_weeks' ).val(); }
			if ( 3 == args.fsel ) {
				args.im = jQuery( 'input#timed_content_rule_monthly_num_of_months' ).val(); }
			if ( 4 == args.fsel ) {
				args.im = jQuery( 'input#timed_content_rule_yearly_num_of_years' ).val(); }
			args.days    = {};
			var days_num = 0;
			jQuery( 'input[id^="timed_content_rule_weekly_days_of_week_to_repeat"]' ).each(
				function() {
			...
			args.ex_days    = {};
			var ex_days_num = 0;
			jQuery( '#timed_content_rule_exceptions_dates option' ).each(
				function() {
		...
		jQuery( '#timed_content_rule_test' ).click(
			function() {
				var args = getArgs();
		...
		jQuery( 'input[id^="timed_content_rule_"], select[id^="timed_content_rule_"]' ).change(
			function(e) {
