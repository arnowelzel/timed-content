...
if (!class_exists('TimedContentPlugin')) {
    ...
    class TimedContentPlugin
    {
         ...
         * more readable. 
         * Also see: https://core.trac.wordpress.org/ticket/25768
         *
         * @param mixed $j
         * @param mixed $req_format
         * @param mixed $i
         * @param bool  $gmt
         * 
         * @return bool|string
        ...
        /**
         * Filter to change sort order of rules to title
         *
         * @param array $query Current query rules
         * 
        ...
        /**
         * Convert localized date/time to English text
         * 
         * @param string $date Date in localized form
         * @param string $time Time
         * 
         * @return string Date and time in English
         */
         ...
         *
         * @param int $current UNIX timestamp of the date/time before incrementing
         * @param int $ordinal Integer symbolizing the ordinal
        ...
        function getNthWeekdayOfMonth( $current, $ordinal, $day )
        {
            ...
            if ($day == 7) { // If $day is 'day of the month', get the day of month based on the ordinal
                switch ($ordinal) {
                case 0:      // First day of the month
                    $the_day = '1';
                    break;
                case 1:      // Second day of the month
                    $the_day = '2';
                    break;
                case 2:      // Third day of the month
                    $the_day = '3';
                    break;
                case 3:      // Fourth day of the month
                    $the_day = '4';
                    break;
                case 4:      // Last day of the month
                ...
                case 0:                   // First 7 days of the month
                    $day_range = range(1, 7);
                    break;
                case 1:                   // Second 7 days of the month
                    $day_range = range(8, 14);
                    break;
                case 2:                   // Third 7 days of the month
                    $day_range = range(15, 21);
                    break;
                case 3:                   // Fourth 7 days of the month
                    $day_range = range(22, 28);
                    break;
                case 4:                   // Last 7 days of the month
        ...
        function getRulePeriods($args)
        {
            ...
            $instance_start = strtotime($this->dateTimeToEnglish($instance_start_date, $instance_start_time) . ' ' . $timezone);
            // End of first occurrence
            $instance_end = strtotime($this->dateTimeToEnglish($instance_end_date, $instance_end_time) . ' ' . $timezone);
        ...
        function getRulePeriodsById($ID, $human_readable = false)
        {
            ...
            elseif ($args['freq'] == 1) $args['interval_multiplier'] = get_post_meta($ID, $prefix . 'daily_num_of_days', true);
            elseif ($args['freq'] == 2) $args['interval_multiplier'] = get_post_meta($ID, $prefix . 'weekly_num_of_weeks', true);
            elseif ($args['freq'] == 3) $args['interval_multiplier'] = get_post_meta($ID, $prefix . 'monthly_num_of_months', true);
            elseif ($args['freq'] == 4) $args['interval_multiplier'] = get_post_meta($ID, $prefix . 'yearly_num_of_years', true);
            ...
            elseif ($freq == 2) {
                if (($days_of_week) && (is_array($days_of_week))) {
                    ...
                    case 1:
                        ...
                        break;
                    case 2:
                        $days_list = sprintf(_x('%1$s and %2$s', 'List of two weekdays', 'timed-content'), $days[0], $days[1]);
                        break;
                    case 3:
                        $days_list = sprintf(_x('%1$s, %2$s, and %3$s', 'List of three weekdays', 'timed-content'), $days[0], $days[1], $days[2]);
                        break;
                    case 4:
                        $days_list = sprintf(_x('%1$s, %2$s, %3$s, and %4$s', 'List of four weekdays', 'timed-content'), $days[0], $days[1], $days[2], $days[3]);
                        break;
                    case 5:
                        $days_list = sprintf(_x('%1$s, %2$s, %3$s, %4$s, and %5$s', 'List of five weekdays', 'timed-content'), $days[0], $days[1], $days[2], $days[3], $days[4]);
                        break;
                    case 6:
                        $days_list = sprintf(_x('%1$s, %2$s, %3$s, %4$s, %5$s, and %6$s', 'List of six weekdays', 'timed-content'), $days[0], $days[1], $days[2], $days[3], $days[4], $days[5]);
                        break;
                    case 7:
                        $days_list = sprintf(_x('%1$s, %2$s, %3$s, %4$s, %5$s, %6$s, and %7$s', 'List of all weekdays', 'timed-content'), $days[0], $days[1], $days[2], $days[3], $days[4], $days[5], $days[6]);
                        break;
                    }
        ...
        function addHeaderCode()
        {
          ...
          * for WP 3.7.1 and under. Echo'd to output.
          * 
          * @return void
          */
...
<style type="text/css" media="screen">
    ...
    .mce-i-timed_content:before {
        ...
        /**
         * Enqueues the JavaScript code necessary for the functionality of the
         * Timed Content Rules management screens.
         * 
         * @return void
         */
        ...
        /**
         * Setup custom fields for rules
         * 
         * @return void
         */
        function setUpCustomFields()
        {
            include_once 'lib/customFields-settings.php';
            include_once 'lib/customFieldsInterface.php';
