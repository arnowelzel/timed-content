...
if ( !class_exists( "timedContentPlugin" ) ) {
	class timedContentPlugin {
					...
					if ( is_array( $matches ) ) {
						foreach ( $matches as $m ) {
							if ( $m[2] == TIMED_CONTENT_CLIENT_TAG ) {
								...
								wp_enqueue_script( 'timed_content_js', WP_PLUGIN_URL . '/timed_content/js/timed_content.js', array( 'jquery' ), '1.0' );
								break 2;
							}
						}
					}
		...
		function registerTinyMCEButton( $buttons ) {
			array_push( $buttons, "separator", "timed_content" );
			return $buttons;
		}
		 
		// Load the TinyMCE plugin : editor_plugin.js (wp2.5)
		function addTimedContentTinyMCEPlugin( $plugin_array ) {
			$plugin_array['timed_content'] = WP_PLUGIN_URL . "/timed_content/tinymce_plugin/editor_plugin_src.js";
			return $plugin_array;
		}
		
		function i18nInit() {
			$plugin_dir = basename( dirname( __FILE__ ) ) . "/lang/";
			load_plugin_textdomain( 'timed_content', null, $plugin_dir );
		}
